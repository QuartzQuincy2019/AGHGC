...
class KeywordsGenerator {
    ...
    get keywords() {
        ...
        if (this.itemCode) {
            ...
            output.add(item.star);
            output.add(lang[LANGUAGE]._Path[item.path]);
            output.add(item.fullName[LANGUAGE]);
            if (getItemType(item) == 'Character') {
                output.add(lang[LANGUAGE].character);
                output.add(lang[LANGUAGE]._CombatType[item.combatType]);
                if (item.params.exclusiveLc) {
                    output.add(findItem(item.params.exclusiveLc).fullName[LANGUAGE]);
                }
            }
                ...
                for (var i = 0; i < CHARACTER_LIST.length; i++) {
                    if (CHARACTER_LIST[i].params.exclusiveLc) {
                        if (CHARACTER_LIST[i].params.exclusiveLc == this.itemCode) {
                            output.add(CHARACTER_LIST[i].fullName[LANGUAGE]);
                            output.add(CHARACTER_LIST[i].code);
                        }
                    }
                }
            }
