 ...
 * @param {Array[Version]} referenceVersions - 参考母版本数组，要求数组已经按照时间排列！
...
function rearrangeAllWarpPools(allPoolsObj, referenceVersions) {
    ...
    for (eachVersion of referenceVersions) {
        let results = ingredients.filter((eachPoolName) => allPoolsObj[eachPoolName].versionInfo.slice(0,3) == eachVersion);
        if (results.length > 0) {
